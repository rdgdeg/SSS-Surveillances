-- INSTALLATION RAPIDE DU SYSTÈME DE VERSIONING
...
BEGIN
    ...
    LOOP
        ...
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = current_table) THEN
            trigger_name := 'trigger_version_' || current_table;
            
            -- Supprimer le trigger s'il existe déjà
            EXECUTE format('DROP TRIGGER IF EXISTS %I ON %I', trigger_name, current_table);
            ...
            EXECUTE format(
                ...
                trigger_name, current_table
            );
            
            RAISE NOTICE 'Trigger créé pour la table: %', current_table;
        ELSE
            RAISE NOTICE 'Table non trouvée (ignorée): %', current_table;
        END IF;
    END LOOP;
END $$;
