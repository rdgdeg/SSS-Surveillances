...
const Card = React.memo(React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={`rounded-lg border bg-card text-card-foreground shadow-sm dark:border-gray-700 dark:bg-gray-800/50 ${className}`}
    {...props}
  />
)));
...
const CardDescription = React.memo(React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(({ className, ...props }, ref) => (
  <p ref={ref} className={`text-sm text-muted-foreground text-gray-500 dark:text-gray-400 ${className}`} {...props} />
)));
