-- INSTALLATION SIMPLE DU SYSTÈME DE VERSIONING
-- Version ultra-simplifiée pour éviter les conflits
...
-- ===== TRIGGERS (un par un pour éviter les conflits) =====

-- Sessions
DROP TRIGGER IF EXISTS trigger_version_sessions ON sessions;
CREATE TRIGGER trigger_version_sessions 
    AFTER INSERT OR UPDATE OR DELETE ON sessions
    FOR EACH ROW EXECUTE FUNCTION trigger_record_version();

-- Creneaux
DROP TRIGGER IF EXISTS trigger_version_creneaux ON creneaux;
CREATE TRIGGER trigger_version_creneaux 
    AFTER INSERT OR UPDATE OR DELETE ON creneaux
    FOR EACH ROW EXECUTE FUNCTION trigger_record_version();

-- Examens
DROP TRIGGER IF EXISTS trigger_version_examens ON examens;
CREATE TRIGGER trigger_version_examens 
    AFTER INSERT OR UPDATE OR DELETE ON examens
    FOR EACH ROW EXECUTE FUNCTION trigger_record_version();

-- Presences enseignants
DROP TRIGGER IF EXISTS trigger_version_presences_enseignants ON presences_enseignants;
CREATE TRIGGER trigger_version_presences_enseignants 
    AFTER INSERT OR UPDATE OR DELETE ON presences_enseignants
    FOR EACH ROW EXECUTE FUNCTION trigger_record_version();

-- Examen auditoires
DROP TRIGGER IF EXISTS trigger_version_examen_auditoires ON examen_auditoires;
CREATE TRIGGER trigger_version_examen_auditoires 
    AFTER INSERT OR UPDATE OR DELETE ON examen_auditoires
    FOR EACH ROW EXECUTE FUNCTION trigger_record_version();

-- Consignes secretariat
DROP TRIGGER IF EXISTS trigger_version_consignes_secretariat ON consignes_secretariat;
CREATE TRIGGER trigger_version_consignes_secretariat 
    AFTER INSERT OR UPDATE OR DELETE ON consignes_secretariat
    FOR EACH ROW EXECUTE FUNCTION trigger_record_version();

-- Soumissions disponibilites
DROP TRIGGER IF EXISTS trigger_version_soumissions_disponibilites ON soumissions_disponibilites;
...
DROP TRIGGER IF EXISTS trigger_version_demandes_modification ON demandes_modification;
CREATE TRIGGER trigger_version_demandes_modification 
    AFTER INSERT OR UPDATE OR DELETE ON demandes_modification
    FOR EACH ROW EXECUTE FUNCTION trigger_record_version();

-- Surveillants
DROP TRIGGER IF EXISTS trigger_version_surveillants ON surveillants;
CREATE TRIGGER trigger_version_surveillants 
    AFTER INSERT OR UPDATE OR DELETE ON surveillants
    FOR EACH ROW EXECUTE FUNCTION trigger_record_version();

-- Admin users
DROP TRIGGER IF EXISTS trigger_version_admin_users ON admin_users;
...
BEGIN
    ...
    IF test_version_id IS NOT NULL THEN
        ...
        -- Compter les versions de test
        SELECT COUNT(*) INTO version_count FROM data_versions WHERE record_id = 'test_record';
        RAISE NOTICE '✅ Versions trouvées: %', version_count;
...
BEGIN
    ...
    ELSE
        RAISE WARNING '⚠️  ATTENTION: Installation incomplète, vérifiez les erreurs ci-dessus';
    END IF;
    RAISE NOTICE '';
END $$;
